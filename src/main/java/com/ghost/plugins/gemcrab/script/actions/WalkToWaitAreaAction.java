...
@Slf4j
public class WalkToWaitAreaAction extends BaseScriptNode implements ActionNode {

    private final MovementService movementService;

    @Inject
    public WalkToWaitAreaAction(Client client, ScriptContext context, MovementService movementService) {
    ...
    public BehaviorResult performAction() {
        ...
        if (client.getLocalPlayer().getWorldLocation().distanceTo(GemCrabScript.WAIT_AREA) < 2) {
            return BehaviorResult.SUCCESS;
        }

        context.setStatus("Walking to wait area");
        if (movementService.walkTo(GemCrabScript.WAIT_AREA)) {
            return BehaviorResult.SUCCESS;
        }
